...
=== 决策树模型可视化呈现及决策树要点理解
可使用Python的Graphviz插件将决策树模型以可视化方式展现。
+ 节点各元素的含义  \
  除了叶子节点外，每个节点都有5个元素：*分裂依据、gini（当前基尼系数）、samples（当前样本数）、value（样本中个类别的数量）、class（分类类别）*。
+ 节点划分依据的合理性验证   \
  机器通过不断训练和计算获得的最优解，使得基尼系数下降值最大。
+ 叶子节点停止分裂的依据  \
  叶子节点停止分裂的依据主要有两个：已经分裂结束，无法再分裂；或者达到限定的分裂条件。
